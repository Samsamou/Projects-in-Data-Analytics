...
In the context of our course in Project in Data Analytics for Decision Making, we will perform an complete analysis of the dataset GermanCredit.csv that we will rename later "Bank Credit". First of all, we will try to understand every variable of this dataset and its trend by using multiple statistical tools. Secondly, we will prepare the data for
...
How should we modify those values ? 
...
```
Those frequency histogram confirms us the problems with had earlier with our 2 variables `MALE_SINGLE`and `GUARANTOR`. 
...
We will now run a multicollinearity test to see if there are any problems with my variable. 
```{r}
vif_model <- vif(model)
print(vif_model)
```
As we can see, there is no problem with the variables except for `OWN_RES`, which has a value above 5. Let's see the updated model:

```{r}
vif_model <- update(model, . ~ . - OWN_RES)
summary(vif_model)
```
...
cat("Accuracy:", accuracy, "\n")
```
We can see that even after running a stepwise regression, there is no difference in the accuracy of the model. 

We now calculate the AUC for the 2 models: 
```{r}
library(pROC)
...
```
The values for AUC are both above 0.75, making them fair to predict good or bad credit. The logistic model has a higher ability to correctly predict than the stepwise, but the difference is not that big. The stepwise has an AIC of 657.15 while the normal logistic regression has a value of 675.23 which is not that of a big difference. Since they have the same accuracy, we can easily say that using a simple logistic or a stepwise does not make that much of a difference. The small improvement in accuracy is not worth the complexity. 
The cross-validation accuracy for the logistic regression model is 0.7632, which is slightly higher than the 0.75 accuracy we obtained earlier with a single train-test split. This indicates that the logistic regression model is somewhat consistent in its performance across different subsets of the data, and we can conclude the same for the stepwise model. 
