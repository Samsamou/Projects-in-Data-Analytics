...
credit_data <- credit_data %>%
  select(-MALE_SINGLE, -MALE_DIV, -MALE_MAR_or_WID, -MARITAL_STATUS, -TELEPHONE, -FOREIGN, -AGE)
```
...
credit_data <- credit_data %>%
  select(RESPONSE, CHK_ACCT, DURATION, HISTORY, NEW_CAR, USED_CAR, FURNITURE, AMOUNT, SAV_ACCT, INSTALL_RATE, GUARANTOR, PRESENT_RESIDENT, REAL_ESTATE, OTHER_INSTALL)
...
# Make predictions on the test dataset with type = "prob"
pred_lin_roc <- predict(svm_model_lin, test_data, type = "prob")
pred_radial_roc <- predict(svm_model_radial, test_data, type="prob")
pred_poly_roc <- predict(svm_model_poly, test_data, type = "prob")
...
roc_obj_2 <- roc(test_data$RESPONSE, positive_pred_radial)
plot(roc_obj_2)
```
