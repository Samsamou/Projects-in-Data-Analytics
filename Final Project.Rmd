...
ggplot(corr_melt, aes(X1, X2, fill = value)) +
  geom_tile() +
...
credit_data <- credit_data %>%
  select(RESPONSE, CHK_ACCT, DURATION, HISTORY, NEW_CAR, USED_CAR, FURNITURE, AMOUNT, SAV_ACCT, INSTALL_RATE, GUARANTOR, PRESENT_RESIDENT, REAL_ESTATE, OTHER_INSTALL, TELEPHONE, FOREIGN, MARITAL_STATUS)
...
print(cm_qda)
```
Lets try to give more weight ti bad customers.
```{r}
 
# Determine class weights
class_weights <- ifelse(train_data$RESPONSE == "Class0", 1, 0.5)
...
# Let's take the probabilities for the positive class (assuming "1" is your positive class)
positive_class_probs <- predictions_cart_probs$'Good'
...
```{r}
prev.class.lin<-predict(svm_model_lin, newdata=test_data)
prev.class.poly<-predict(svm_model_poly, newdata=test_data)
prev.class.radial<-predict(svm_model_radial, newdata=test_data)
prev.class.lin[1:10]
...
Class probabilities:
```{r}
prev.prob.lin<-predict(svm_model_lin, newdata=test_data, type="prob")
prev.prob.poly<-predict(svm_model_poly, newdata=test_data, type="prob")
prev.prob.radial<-predict(svm_model_radial, newdata=test_data, type="prob")
round(head(prev.prob.lin),3)
round(head(prev.prob.poly),3)
round(head(prev.prob.radial),3)
```

ROC Curves:
```{r}
prev.class<-data.frame(lin=prev.class.lin,
                       poly=prev.class.poly, 
                       radial=prev.class.radial, 
                       obs=test_data$RESPONSE)
prev.class %>% summarise_all(funs(err=mean(obs!=.))) %>% select(-obs_err) %>% 
  round(3)

prev.prob<-data.frame(lin=prev.prob.lin[,2],
                      poly=prev.prob.poly[,2], 
                      radial=prev.prob.radial[,2], 
                      obs=test_data$RESPONSE)

df.roc<-prev.prob %>% gather(key=Methode, value=score, lin, poly, radial)
basicplot <- ggplot(df.roc) + aes(d=obs, m=score, color=Methode) + geom_roc()
basicplot + style_roc(theme = theme_light) + 
