...
set.seed(42)
lda_model <- train(RESPONSE ~ ., data = train_data, method = "lda", trControl = ctrl)
...
# Fit the QDA model with regularization
set.seed(42)
qda_model <- qda(RESPONSE ~ ., data = train_data, prior = c(0.5, 0.5), k = 0.5)  # k is the regularization parameter

# Make predictions on the test dataset
predictions_qda <- predict(qda_model, test_data)$class

# Evaluate the model using a confusion matrix
cm_qda <- confusionMatrix(as.factor(predictions_qda), as.factor(test_data$RESPONSE))
...
```
As we can see, the accuracy of the model is mediocre at best. With 0.71, and a sensitivity of 0.62, the model is essentially tossing a coin-flip for every bad client. What if we tried upsampling?
...
print(cm_nn)
```
The results are now more satisfactory and the confusion matrix shows us an improvement of almost 10%, which is really good. Now let's do it with fewer variables by making a stepwise selection:
