...
nn_model <- train(RESPONSE ~ ., 
                  ...
                  trControl = ctrl,
                  epochs = 2,
                  tuneLength = 5) #tuneLength = 100, metric = "Accuracy") to get a model that spits out a 93% accuracy
...
```
After running a big analysis, we can see that the accuracy is now at 0.72 and the sensitivity is a 0.75. The overall accuracy is better too, so the model is now able to handle better the difference between the two types of clients.

Here is the last model, plotted:
