...
```{r eval=FALSE, include=FALSE}
# Perform stepwise regression with cross validation
...
stepwise_model_cv <- train(RESPONSE ~ ., data = train_data, method = "glmStepAIC", 
                           trControl = ctrl, 
...
# Summary of the final model
summary(stepwise_model_cv)
# Predict on the test data
test_probs_cv <- predict(stepwise_model_cv, newdata = test_data, type = "prob")[,2]
test_preds_cv <- ifelse(test_probs_cv > 0.5, 1, 0)

# Calculate accuracy
confusion_matrix_cv <- table(Predicted = test_preds_cv, Actual = test_data$RESPONSE)
